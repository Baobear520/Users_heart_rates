...
def populate_tables(num_of_users,num_of_hr_records):
    ...
    """
    validate_populate_table_params(num_of_users,num_of_hr_records)
    ...
    with engine.connect() as connection:
        ...
        user_id_scalar_subq = select(users_table.c.id).scalar_subquery()
        ...
        connection.execute(insert(heart_rates_table).values(
            heart_rates_table.c.user_id == user_id_scalar_subq,
            heart_rates)
        )
        connection.commit()
    ...
    with engine.connect() as connection:
        ...
        subquery = (
            select(
                ...
                cast(
                    func.round(cast(func.avg(heart_rates_table.c.heart_rate), Numeric), 2), Float)
